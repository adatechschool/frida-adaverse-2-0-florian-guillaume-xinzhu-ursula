...
export default function ProjectListAdmin({ projects }: Props) {
  ...
  // Regroupement par catégorie Ada
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {sortedCategories.map(([adaName, projectsList]) => (
          <div key={adaName} className="mb-16">
            ...
            {/* Grille des cards admin */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {projectsList.map((project) => (
                <ProjectCardAdmin
                  key={project.students_projects.id}
                  projectId={project.students_projects.id}   // ✅ ADAPTATION IMPORTANTE
                  project={project}                          // ✅ si tu veux afficher infos dans la card admin
                />
              ))}
            </div>
